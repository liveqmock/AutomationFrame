var YDataGrid = function(config){
		...
		var Handler = {
			...
			edit:  function(callback){
				...
				if (Utils.checkSelectOne(record)){
					...
					CEYND.getById(Action.getId,data,function(result){
						CEYND.closeProgress();
				...
				if (Utils.checkSelect(records)){
					$.messager.confirm('Confirm','Are you sure you want to delete record?',function(r){  
					    if (r){
					    	...
					    	var data = arr.join("&");
					   		CEYND.deleteForm(Action.remove,data,function(result){
								CEYND.closeProgress();
								Events.refresh();
								//回调函数
								if(jQuery.isFunction(callback)){
			...
			save: function(callback){
				if(Form.edit.form('validate')){
					...
					CEYND.saveForm(Form.edit,function(data){
						CEYND.closeProgress();
		...
		var Utils = {
			...
			checkSelectOne : function(rows){//检查grid是否只勾选了一行,是返回 true,否返回true
				...
				CEYND.alert('警告','只能选择一行记录.','warning');  
				return false;
			}
		}
		...
		var initTbar = function(){
			...
			//查询页面授权的btnType
			CEYND.ajaxJson(_url,data,function(data){
				if(data.success){
					if(data.allType){
