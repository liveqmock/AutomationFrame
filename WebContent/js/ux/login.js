$package('CEYND.login');
CEYND.login = function(){
	return {
		toLogin:function(){
			try{
				var form = $("#loginForm");
				if(form.form('validate')){
					CEYND.progress('Please waiting','Loading...');
					CEYND.submitForm(form,function(data){
						CEYND.closeProgress();
						if(data.success){
					 		window.location= "main.shtml";
				        }else{
				       	   CEYND.alert('提示',data.msg,'error');  
				        }
				        CEYND.login.loadVrifyCode();//刷新验证码
					});
				}
			}catch(e){
				
			}
			return false;
		},
		loadVrifyCode:function(){//刷新验证码
			var _url = "ImageServlet?time="+new Date().getTime();
			$(".vc-pic").attr('src',_url);
		},
		init:function(){
			if(window.top != window.self){
				window.top.location =  window.self.location;
			}
			//验证码图片绑定点击事件
			$(".vc-pic").click(CEYND.login.loadVrifyCode);
			
			var form = $("#loginForm");
			form.submit(CEYND.login.toLogin);
		}
	}
}();

$(function(){
	CEYND.login.init();
});		